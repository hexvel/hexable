...
from .methods import (account, ads, adsweb, app_widgets, apps, asr, auth,
                      board, bugtracker, database, docs, donut,
                      downloaded_games, fave, friends, gifts, groups,
                      lead_forms, likes, market, messages, newsfeed, notes,
                      notifications, orders, pages, photos, podcasts, polls,
                      pretty_cards, search, secure, stats, status, storage,
                      store, stories, streaming, translations, users, utils,
                      video, wall, widgets)

if typing.TYPE_CHECKING:
    from hexable import API  # type: ignore
...
class APICategories(ABC):
    ...
    async def execute(self, code: str) -> typing.Any:
        return await self.api_instance.method("execute", {"code": code})
